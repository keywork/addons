local myTip = CreateFrame("GameTooltip","ILockTooltip",nil,"GameTooltipTemplate")
	...
	if ( linkType == "instancelock" ) then
		...
		if mylink then
			if not myTip:IsVisible() and frame:IsVisible() then 
				...
				if side == "left" then
					myTip:SetPoint("TOPRIGHT",frame,"TOPLEFT",0,-10)
				elseif side == "right" then
					myTip:SetPoint("TOPLEFT",frame,"TOPRIGHT",0,-10)
				end
...
ItemRefTooltip:HookScript("OnDragStop",
function(self,button) 
	if myTip:IsVisible() and (myTip:GetParent():GetName()==self:GetName()) then
		...
		myTip:ClearAllPoints()
		if side == "left" then
			myTip:SetPoint("TOPRIGHT",self,"TOPLEFT",0,-10)
		elseif side == "right" then
			myTip:SetPoint("TOPLEFT",self,"TOPRIGHT",0,-10)
		end
	end
end)
