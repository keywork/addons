...
local MINOR_VERSION = 90000 + (tonumber(("$Revision: 109 $"):match("%d+")) or 0)
			...
			if header then
				...
				if numObjectives and numObjectives > 0 then
					...
					for o = 1, numObjectives do
						...
						local numNeeded, numItems, mobName, newDesc
						if qtype == 'item' or qtype == 'object' then --'object' for the leaderboard in Dousing the Flames of Protection; maybe others.
							numItems, numNeeded, newDesc = lib:extractQuestObjectiveByLocale(desc, objects_pattern)
							if newDesc ~= nil then
								desc = newDesc
						...
						elseif qtype == 'monster' then
							numItems, numNeeded, mobName = lib:extractQuestObjectiveByLocale(desc, monsters_pattern)
							if mobName == nil or numItems == nil or numNeeded == nil then
								...
								numItems, numNeeded, mobName = lib:extractQuestObjectiveByLocale(desc, objects_pattern)
							end
							numItems = tonumber(numItems)
							numNeeded = tonumber(numNeeded)
							desc = mobName
						...
						if desc then
							objectives[desc] = new(numItems or 0, numNeeded or 1337, qtype)
...
function lib:extractQuestObjectiveByLocale(desc, pattern)
	local locale = GetLocale()
	local what, numItems, numNeeded
	if locale == "deDE" then
		what, numItems, numNeeded = desc:match(pattern)	
	else
		numItems, numNeeded, what = desc:match(pattern)
	end
	return numItems, numNeeded, what
end
...
local function worth_sending() return lib.worth_sending and (not IsInRaid()) end --and (GetNumGroupMembers() > 0) end
